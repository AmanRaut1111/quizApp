...
const submitAnswer = async (req, res) => {
    try {
        ...
        if (data) {
            res.status(200).json({ message: "Answer submitted sucessfully...!", status: true, statusCode: 200, data: data })
        } else {
...
const getUserQuiz = async (req, res) => {
    try {
        ...
        if (data) {
            res.status(200).json({ message: "Quiz Data found Sucessfully...!", status: true, statusCode: 200, data: data })
        } else {
            res.status(400).json({ message: "Something Went Wrong...!", status: false, statusCode: 400 })
        }
    } catch (error) {
...
const getQuizScore = async (req, res) => {
    try {
                ...
                '$project': {
                    ...
                    'score': {
                        '$cond': {
                            'if': {
                                '$eq': [
                                    '$givenAnswer', '$quizData.correctAnswer'
                                ]
                            },
                            'then': 1,
                            'else': 0
        ...
        if (scoreData) {
            res.status(200).json({ message: "data found Sucessfully...!", status: true, statusCode: 200, data: scoreData })
        } else {
            res.status(400).json({ message: 'Something Went Wrong...!', status: false, statusCode: 400 })
        }
    } catch (error) {
