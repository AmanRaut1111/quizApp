...
const regUser = async (req, res) => {
    try {
        ...
        if (data) {
            res.status(200).json({ message: "User  is Registered Sucessfully...!", status: true, statusCode: 200, data: data, token: token })
        } else {
        ...
        if (email, password) {
            ...
            if (checkUser) {
                ...
                if (matchpassword) {
                    ...
                    res.status(400).json({ message: "Password does not Match..!", status: false, statusCode: 400 })
                }
            } else {
                res.status(400).json({ mesage: "User is Not Found", status: false, statuCode: 400 })
            }
        } else {
